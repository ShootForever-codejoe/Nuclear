...
public final class RenderUtil {
    ...
    public static void renderEntityBoundingBox(@NotNull PoseStack poseStack, int type, @NotNull Entity entity, int color, boolean damage) {
    ...
    public static int rainbowDraw(long speed, long @NotNull ... delay) {
        long time = System.currentTimeMillis() + (delay.length > 0 ? delay[0] : 0L);
        return Color.getHSBColor((float) (time % (15000L / speed)) / (15000.0F / (float) speed), 1.0F, 1.0F).getRGB();
    }

    public static void drawRect(@NotNull PoseStack poseStack, int left, int top, int right, int bottom, int color) {
        if (left < right) {
            int j = left;
            left = right;
            right = j;
        ...
        bufferbuilder.vertex(matrix, (float) right, (float) bottom, 0.0F).color(color >> 16 & 0xFF, color >> 8 & 0xFF, color & 0xFF, color >> 24 & 0xFF).endVertex();
    ...
    public static void drawGradientRect(@NotNull PoseStack poseStack, int left, int top, int right, int bottom, int startColor, int endColor) {
        Matrix4f matrix = poseStack.last().pose();
        Tesselator tessellator = Tesselator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuilder();
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
